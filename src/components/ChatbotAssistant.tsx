...
• 📍 Location: Ampara, Sri Lanka`;
    ...
    if (q.includes('public pulse') || q.includes('nlp') || q.includes('sarcasm') || q.includes('civic') || q.includes('sinhala') || q.includes('political') || q.includes('roberta') || q.includes('grounded')) {
      return `Malith co-built **Public Pulse 🇱🇰**, a completed grounded LLM system fine-tuned on XLM-RoBERTa for Sri Lankan TV-news YouTube discourse:
• A 3-layer cascade (Utility ➔ Topic ➔ Stance) trained on a 10,000-row golden dataset, reaching macro-F1 of **0.98 / 0.72 / 0.86**.
• A deterministic BM25 evidence-retrieval layer feeds a Gemini-powered Grounded LLM that can only cite retrieved evidence.
• Every generated claim is checked by a 3-stage **faithfulness-verification engine**, backed by 171 passing automated tests!`;
    }
